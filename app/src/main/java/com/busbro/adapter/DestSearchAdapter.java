...
public class DestSearchAdapter extends ArrayAdapter<String> {
    private Filter mFilter;

    private List<String> mSubData = new ArrayList<String>();
    static int counter=0;
    ArrayList<BusStop> sortings;
    public DestSearchAdapter(final Context context, int textViewResourceId) {
            ...
            protected FilterResults performFiltering(CharSequence constraint) {
                ...
                if(constraint != null) {
                    try {
                        ...
                        sortings = nearestBus.searchDestBuses(context, jsArray);
                        Log.d("SearchString",""+sortings);
                        for(BusStop busStop :sortings){
                            try{
                                if (busStop.getDescription().toLowerCase().equalsIgnoreCase(constraint.toString().toLowerCase())) {
